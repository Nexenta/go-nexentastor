pipeline {
    parameters {
        ...
        string(name: 'TEST_NS_HA_1',   defaultValue: '',                          description: 'NS HA cluster 1 node API address', trim: true)
        string(name: 'TEST_NS_HA_2',   defaultValue: '',                          description: 'NS HA cluster 2 node API address', trim: true)
    }
    options {
    ...
    stages {
        ...
        stage('Tests [e2e-ns-cluster]') {
            when {
                allOf {
                    expression { env.TEST_NS_HA_1 != '' }
                    expression { env.TEST_NS_HA_1 != null }
                    expression { env.TEST_NS_HA_2 != '' }
                    expression { env.TEST_NS_HA_2 != null }
                }
            }
            steps {
